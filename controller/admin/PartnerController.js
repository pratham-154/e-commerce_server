...
const add = async (req, res) => {
  ...
  if (!validatorRules.fails()) {
    let resp = await partnerModel.insert(data);
    if (resp) {
      res.send({
        status: true,
...
const update = async (req, res) => {
  ...
  if (!validatorRules.fails()) {
    let resp = await partnerModel.update(id, data);
    if (resp) {
      res.send({
        status: true,
...
const remove = async (req, res) => {
  ...
  let resp = await partnerModel.remove(id);
...
const view = async (req, res) => {
  ...
  let resp = await partnerModel.get(id);
...
const bulkAction = async (req, res) => {
  ...
  if (ids && ids.length > 0 && type) {
    switch (type) {
      case "active":
        await partnerModel.updateAll(ids, {
          status: 1,
        });
      ...
      case "inactive":
        ...
        break;
      case "delete":
        await partnerModel.removeAll(ids);
