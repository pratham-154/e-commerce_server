...
const getAll = async (
  ...
  try {
    let listing = cart.find(where, select).sort(orderBy);
...
const getRow = async (where, select = []) => {
  try {
    let record = await cart.findOne(where, select).exec();
    return record;
  } catch (error) {
    console.log(error);
    return false;
...
const updateAll = async (ids, data) => {
  try {
    let resp = await cart.updateMany(
      {
        _id: { $in: ids },
      },
      data
...
const removeAll = async (ids) => {
  try {
    let record = await cart.deleteMany({
      _id: { $in: ids },
    });
    return record;
  } catch (error) {
...
const getCounts = async (where = {}) => {
  try {
    let record = await cart.countDocuments(where).exec();
    return record;
  } catch (error) {
    return false;
  }
