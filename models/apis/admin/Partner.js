...
const getAll = async (
  ...
  try {
    let listing = partner.find(where, select).sort(orderBy);
...
const getRow = async (where, select = []) => {
  try {
    let record = await partner.findOne(where, select).exec();
    return record;
  } catch (error) {
    console.log(error);
    return false;
...
const updateAll = async (ids, data) => {
  try {
    let resp = await partner.updateMany(
      {
        _id: { $in: ids },
      },
      data
...
const removeAll = async (ids) => {
  try {
    let record = await partner.deleteMany({
      _id: { $in: ids },
    });
    return record;
  } catch (error) {
