...
const getAll = async (
  ...
  try {
    let listing = banner.find(where, select).sort(orderBy);
...
const get = async (id, select = [], joins = []) => {
  try
  {   
      let record = banner.findById(id,select);
...
const getRow = async (where, select = []) => {
  try {
    let record = await banner.findOne(where, select).exec();
    return record;
  } catch (error) {
    console.log(error);
    return false;
...
const update = async (id, data) => {
  try
  {
      ...
      let resp = await banner.updateOne(
          {
              "_id":id
          },
          data
...
const updateAll = async (ids, data) => {
  try {
    let resp = await banner.updateMany(
      {
        _id: { $in: ids },
      },
      data
...
const removeAll = async (ids) => {
  try {
    let record = await banner.deleteMany({
      _id: { $in: ids },
    });
    return record;
  } catch (error) {
...
const getCounts = async (where = {}) => {
  try {
    let record = await banner.countDocuments(where).exec();
    return record;
  } catch (error) {
    return false;
  }
