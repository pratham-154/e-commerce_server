...
const getAll = async (
  ...
  try {
    let listing = contactUs.find(where, select).sort(orderBy);
...
const get = async (id, select = [], joins = []) => {
  try
  {   
      let record = contactUs.findById(id,select);
...
const getRow = async (where, select = []) => {
  try {
    let record = await contactUs.findOne(where, select).exec();
    return record;
  } catch (error) {
    console.log(error);
    return false;
...
const update = async (id, data) => {
  try
  {
      ...
      let resp = await contactUs.updateOne(
          {
              "_id":id
          },
          data
...
const updateAll = async (ids, data) => {
  try {
    let resp = await contactUs.updateMany(
      {
        _id: { $in: ids },
      },
      data
...
const removeAll = async (ids) => {
  try {
    let record = await contactUs.deleteMany({
      _id: { $in: ids },
    });
    return record;
  } catch (error) {
...
const getCounts = async (where = {}) => {
  try {
    let record = await contactUs.countDocuments(where).exec();
    return record;
  } catch (error) {
    return false;
  }
