...
const productCategoryModel = require("../../models/apis/admin/ProductCategory");
...
const add = async (req, res) => {
  ...
  if (!validatorRules.fails()) {
    let resp = await productCategoryModel.insert(data);
    if (resp) {
      res.send({
        status: true,
...
const update = async (req, res) => {
  ...
  if (!validatorRules.fails()) {
    let resp = await productCategoryModel.update(id, data);
    if (resp) {
      res.send({
        status: true,
        message: "Record has been saved successfully.",
...
const remove = async (req, res) => {
  ...
  let resp = await productCategoryModel.remove(id);
...
const bulkAction = async (req, res) => {
  ...
  if (ids && ids.length > 0 && type) {
    switch (type) {
      case "active":
        await productCategoryModel.updateAll(ids, {
          status: 1,
        });
      ...
      case "inactive":
        ...
        break;
      case "delete":
        await productCategoryModel.removeAll(ids);
